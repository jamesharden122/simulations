...
fn gen_z_vec(time_steps: Int) -> Tensor[DType.float64]:
    ...
    print(time_delta)
    var shape = TensorShape(time_steps)
    var z_norm = Tensor[DType.float64].randn(shape, mean, stand_dev)
    return(z_norm)
...
struct BrownianBridge:
    ...
    fn __init__(inout self, power: Int, time_span: Int, tens_dict: TensorDict):
        self.power = power
        self.time_steps = 2**power
        self.time_span = time_span
        self.tens_dict = tens_dict                                         

    fn generate_bridge(inout self) -> Tensor[DType.float64]:
        seed()
...
struct GeomBM:
    ...
    def gen_weiner_paths(inout self):
        var brown_brdg = BrownianBridge(24,1, TensorDict())
        var t_vec = gen_time_vec(self.time_steps)
        self.sim_paths.set("Time", t_vec)
        for i in range(self.num_paths):
            self.sim_paths.set(str(i), brown_brdg.generate_bridge())
            print("Generated Path", i)

    def gen_mcs_lazy(inout self) -> Tensor[DType.float64]:
        ...
        for i in range(0,self.num_paths-1):
            tmp = self.sim_paths.get[DType.float64](str(i))
